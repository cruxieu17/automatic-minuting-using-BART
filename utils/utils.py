...
def clean_punc(ctx):
    puncs = json.load(open('/content/drive/MyDrive/Journal/utils/clean_punc.json', 'r'))
    temp = ''
    while temp!=ctx:
        temp = ctx
...
def rem_fillers(ctx):
    fill_file = open('/content/drive/MyDrive/Journal/utils/fillers.txt','r')
...
def rem_stopwords(ctx):
    stopfile = open('/content/drive/MyDrive/Journal/utils/stopwords.txt','r')
